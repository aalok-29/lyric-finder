...
function Lyrics() {
    ...
    const{artist,track,searchState,setLyricsState}=useContext(LyricsContext)
        ...
        function getlyrics()
        {
//             fetch(`https://cors-anywhere.herokuapp.com/https://api.musixmatch.com/ws/1.1/matcher.lyrics.get?q_track=${track}&q_artist=${artist}&apikey=1538e2542ebac4878de127b5193d9854`).then((res) => {
//     setLyrics(res.data.message.body.lyrics.lyrics_body);
    
//   })
            axios.get(`https://cors-anywhere.herokuapp.com/https://api.musixmatch.com/ws/1.1/matcher.lyrics.get?q_track=${track}&q_artist=${artist}&apikey=${process.env.REACT_APP_MM_KEY}`).then(res => {
           setLyrics(res.data.message.body.lyrics.lyrics_body);
        })
